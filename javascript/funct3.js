...
function DelimitateBlocks(lines, format) {
	...
	if (format=="TRANSFAC"){
		var i =0;
		for (line_index=0; line_index <= (lines.length -1); line_index++) {
			if (lines[line_index].slice(0,2)=="AC") {
				initBlock[i] = line_index;
				var k = line_index+1;
				while ((k<(lines.length -1)) && (lines[k].slice(0,2)!="//")) {
					k++;
				}
				if ((k==(lines.length -1)) && (lines[k].slice(0,2)!="//")) {		// It's the end of file, but the block terminator "//"
					trash = initBlock.pop();										// is not found -> last block incomplete (file.size>1MB)
				}else{
					endBlock[i]=k;													// The file ends correctly with the block terminator "//"			
				}
				i++;
			}
		}
...
function RemoveSpaces(lines, init, end) {									// Create an array of lists. Each list has the column numbers of his row.
	...
	for (var i=(init+1); i<=end; i++) {
		myArray[i] = (lines[i].split(" ").length);
	}
	return [myArray];
}

function BracketsChar(myBlock) {
	
	return myBlock;
}

function CheckColumnsNumber(myBlock) {
	
	var stillOK = 1;
	var ref = myBlock[1];
	for (var l in myBlock) {
...
function ValidateBlocks(lines, init, end, format) {
	...
	if (format == "RAW"){
		...
		stillOk = CheckColumnsNumber(myBlock);
	}else if (format == "JASPAR") {
		var myBlock = [];	
		myBlock = RemoveSpaces(lines, init, end);
		//myBlock = BracketsChar(myBlock);
		stillOK = CheckColumnsNumber(myBlock);
	}else {
...
function validateMatrix(data, file, validatingDIV, fileid, uploadBUTTON) {
	...
	if (format== "ERROR") {
		...
		if (Blocks[0] == [])	{							//No header founded for the format (CHECK POSSIBLE ERROR: Blocks[0].length==0 instead????)
			...
			var i=0;
			while ( (stillOK >= 0)&&(i <= Blocks[0].length) ) {
