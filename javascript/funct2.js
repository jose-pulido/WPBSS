...
function LocateHeaders(lines, format) {
	var header_index = 0;
	var myHeaders = [];
...
function CheckRAWline(myline, ROWsize){
	...
	numbersARRAY = spacesPurge(numbersARRAY);
	if  (numbersARRAY.length != ROWsize) {							//All rows must have the same number of columns
		var correctline = -1;									
	}else{
		...
		if (!validNUMBER) {								//Matrix elements must be numbers
			correctline = -2;		
		}else{
			correctline = 1;
		}
...
function validateMatrix(data, file, validatingDIV, fileid, uploadBUTTON) {
	...
	if (file.size<validate_size){								//BUSCAR CABECERAS TAMBIÉN PARA TRANSFAC
		var lastHeader = myHeaders[myHeaders.length -1];		//CASOS SÓLO HAYA UN HEADER O NO HAYA NINGUNO!!!!!!!!!!
	}else{
